...
class Noise(Attack):
    ...
    def generate(self, x, **kwargs);
        ...
        eta = torch.FloatTensor(*x.shape).uniform_(-self.eps, self.eps, dtype=self.dtype)
    ...
    def parse_params(self, eps=0.3, ord=np.inf, clip_min=None, clip_max=None):
        """
        Take in a dictionary of parameters and applies attack-specific checks before saving them
        as attributes
        ...
        if self.ord not in [np.inf]:
            raise ValueError("Norm order must be in np.inf")
